...
#include "../model/loot/loot_data.h"
...
model::Game LoadGame(const std::filesystem::path& json_path) {
  ...
  model::Game game{time_interval, probability};
 
  double default_dog_speed = ApplyDogSpeed(map_json, std::string(config::MapKey::DEFAULT_DOG_SPEED),
    model::DEFAULT_DOG_SPEED);
  

  for (auto map_object : map_json.as_object().at(std::string(config::MapKey::MAPS)).as_array()) {
