...
json::value ApiRequestHandler::SerializeMapInfo(std::string_view id) {
  if (const auto map = app_.FindMap(model::Map::Id{ std::string{id} })) {
    ...
    array_loot_types.reserve(loot_types.size());
...
json::value ApiRequestHandler::SerializePlayersInfoAndLostObject(const Players& players, const std::vector<model::LostObject>& lost_objects) {
  ...
  for (const auto& player : players) {
    if (auto p = player.lock()) {
      ...
      player_info[std::string(config::MapKey::POSITION)] = { p->GetDog().lock()->GetPosition().x, p->GetDog().lock()->GetPosition().y };
      player_info[std::string(config::MapKey::SPEED)] = { p->GetDog().lock()->GetSpeed().x, p->GetDog().lock()->GetSpeed().y };
      auto dir = p->GetDog().lock()->GetDirection();
      std::string dir_str;
